...
- Add a new `strict` keyword argument to all `decode` functions,
  ...
  ({pr}`434`).
- Support all supported types as inputs to `msgspec.convert` ({pr}`431`,
  {pr}`418`).
...
- Add new experimental `msgspec.inspect` module for inspecting type
  ...
  based on msgspec-compatible types. See the docs for more
  information ({pr}`253`).
...
- Add a new `msgspec.Raw` type for delayed decoding of message fields /
...
- Improve performance of `nogc=True` structs.
  ...
  per instance. Also added a benchmark for how `msgspec` can interact with
